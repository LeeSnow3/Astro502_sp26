    ...
    def get_array_grids(self, recalc=False):
        ...
        if calculate:
            if self.eep_replaces == "age":
                # existing track-grid behavior (leave as-is)
                ii0 = self.fehs
                ii1 = self.masses
                xname = "age"
                yname = "dt_deep"
            elif self.eep_replaces == "mass":
                # ✅ isochrone-grid behavior
                ii0 = self.ages   # or self.log10_isochrone_ages, depending on attribute name
                ii1 = self.fehs
                xname = "mass"
                yname = "dm_deep"
            else:
                raise ValueError(self.eep_replaces)

            n = len(ii0) * len(ii1)
            x_arrays = np.zeros((n, self.n_eep)) * np.nan
            y_arrays = np.zeros((n, self.n_eep)) * np.nan
            lengths = np.zeros(n) * np.nan

            for i, (x0, x1) in tqdm(
                enumerate(itertools.product(ii0, ii1)), total=n, desc=f"building irregular {xname} grid"
            ):
                subdf = self.df.xs((x0, x1), level=(0, 1))
                xs = subdf[xname].values
                lengths[i] = len(xs)

                x_arrays[i, :len(xs)] = xs
                y_arrays[i, :len(xs)] = subdf[yname].values

            np.savez(self.array_grid_filename, **{xname: x_arrays, yname: y_arrays, "lengths": lengths.astype(int)})

        d = np.load(self.array_grid_filename)
        # Return in a consistent order expected by callers:
        if self.eep_replaces == "mass":
            return d["mass"], d["dm_deep"], d["lengths"]
        else:
